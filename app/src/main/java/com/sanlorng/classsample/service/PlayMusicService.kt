...
import com.sanlorng.classsample.fragment.NotificationFragment
import com.sanlorng.classsample.helper.bitmap
...
class PlayMusicService : Service() {
    ...
    override fun onCreate() {
        ...
        pauseIntent = PendingIntent.getBroadcast(this,NotificationFragment.MUSIC_CHANNEL_INT,
            Intent(ACTION_PAUSE),PendingIntent.FLAG_UPDATE_CURRENT)
        playIntent = PendingIntent.getBroadcast(this,NotificationFragment.MUSIC_CHANNEL_INT,
            Intent(ACTION_PLAY),PendingIntent.FLAG_UPDATE_CURRENT)
        val playLastAction = NotificationCompat.Action.Builder(
            R.drawable.ic_skip_previous_black_24dp,
            getString(R.string.playPrevious),
            PendingIntent.getBroadcast(this,NotificationFragment.MUSIC_CHANNEL_INT,
                Intent(ACTION_PREV),PendingIntent.FLAG_UPDATE_CURRENT))
            .build()

        val playNextAction = NotificationCompat.Action.Builder(
            R.drawable.ic_skip_next_black_24dp,
            getString(R.string.playNext),
            PendingIntent.getBroadcast(this,NotificationFragment.MUSIC_CHANNEL_INT,
                Intent(ACTION_NEXT),PendingIntent.FLAG_UPDATE_CURRENT))
            .build()
        ...
        notificationBuilder = NotificationCompat.Builder(this,NotificationFragment.MUSIC_CHANNEL)
            .setSmallIcon(R.drawable.ic_music_note_black_24dp)
            .setSubText("音乐")
            .setShowWhen(false)
            .setContentIntent(PendingIntent.getActivity(this,NotificationFragment.MUSIC_CHANNEL_INT
            , Intent(this,MusicPlayActivity::class.java),PendingIntent.FLAG_UPDATE_CURRENT
            ))
    ...
    fun createNotificationChannel() {
        getSystemService(NotificationManager::class.java).run {
            createNotificationChannel(NotificationChannel(NotificationFragment.MUSIC_CHANNEL,"音乐控制", NotificationManager.IMPORTANCE_LOW).apply {
    ...
    companion object {
        private const val PACKAGE_NAME = "com.sanlorng.classsample"
    ...
    inner class PlayMusicBinder: Binder() {
        ...
        private fun switchPlayStatus(play: Boolean) {
            if (play) {
                ...
                startForeground(NotificationFragment.MUSIC_CHANNEL_INT,notificationBuilder.build())
            }else {
                ...
                NotificationManagerCompat.from(this@PlayMusicService)
                    .notify(NotificationFragment.MUSIC_CHANNEL_INT,notificationBuilder.build())
        ...
        private fun loadAlbumCover(path: String):Bitmap? {
            return MediaMetadataRetriever().run {
                if (path.startsWith("android.resource://$packageName/"))
                    ...
                    setDataSource(path)
                if (embeddedPicture != null) BitmapFactory.decodeByteArray(embeddedPicture,0,embeddedPicture.size) else null
            }
        }
        ...
        fun onPlayPrepared(mediaPlayer: MediaPlayer) {
            ...
            playingMusic.apply {
                metaDataBuilder.putString(MediaMetadataCompat.METADATA_KEY_ARTIST,artist)
                    ...
                    .putBitmap(MediaMetadataCompat.METADATA_KEY_ALBUM_ART,albumCover?:getDrawable(R.drawable.drawable_gradient)?.bitmap)
                    .putLong(MediaMetadataCompat.METADATA_KEY_DURATION,mediaPlayer.duration.toLong())
                mediaSession.setMetadata(metaDataBuilder.build())
                notificationBuilder
                    .setContentTitle(title)
                    .setContentText("$artist - $album")
                    .setLargeIcon(albumCover ?: getDrawable(R.drawable.drawable_gradient)?.bitmap)
                switchPlayStatus(true)
            }
